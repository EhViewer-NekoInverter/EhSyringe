...
merge(
    ...
    undefined,
    {
        ...
        ' Default port clients only (Slower. Use only if behind a firewall or proxy that blocks outgoing non-standard ports.)':
            ' 仅使用默认端口的客户端 (稍慢，仅当防火墙或代理阻止非标准端口的流量时启用此项)',
        ...
        ', or use a VPN located there, and will default to load images from the ': '，或者使用了位于此处的 VPN，默认会从 ',
        ...
        'Images are normally resampled to 1280 pixels (desktop) or 800 pixels (mobile) horizontal resolution for online viewing. You can select one of the following alternative resolutions. (Resolutions above 1280x are restricted to accounts with any hath perk, a bronze star, or an UID below 3,000,000.)':
            '通常情况下，图像会被重采样到 1280 像素 (桌面端) 或 800 像素 (移动端) 宽度用于在线浏览，您也可以手动指定重采样分辨率。 (高于 1280 像素只提供给解锁过任意 Hath Perk、铜星或 UID 小于 3,000,000 的用户)',
        'Images are normally resampled to 1280 pixels (desktop) or 800 pixels (mobile) horizontal resolution for online viewing. You can select one of the following alternative resolutions.':
            '通常情况下，图像会被重采样到 1280 像素 (桌面端) 或 800 像素 (移动端) 宽度用于在线浏览，您也可以手动指定重采样分辨率。',
        ' Auto': ' 自动',
